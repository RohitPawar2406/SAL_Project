...
# ----------------------------------------------------
DATASET_PATH = "/scratch/rohit.pawar/sal_dir_marathi"
BASE_MODEL = "sumedh/wav2vec2-large-xlsr-marathi"

# --- Experiment Sample & Split ---
# This is shared for both experiments to ensure a fair comparison
DATASET_SAMPLE_FRACTION = 0.001 # Use 0.1% of the data (Adjust as needed)
TEST_SPLIT_SIZE = 0.1           # Use 10% of that sample for evaluation

# --- Main Output Directory ---
MAIN_OUTPUT_DIR = "/scratch/rohit.pawar/runs_sal/sal_marathi_experiment"
...
NUM_PROC = 1                  # Number of cores for data preprocessing
...
def create_comparison_plots(history_devanagari: List[Dict], 
    ...
    try:
        ...
        plt.title("Experiment 1: Devanagari Head (Marathi)")
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.legend()
        plt.grid(True)
        ...
        plt.title("Experiment 2: Latin Head (Marathi)")
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.legend()
        plt.grid(True)
    ...
    try:
        ...
        plt.title("Marathi CER Comparison (Same Audio, Different Heads)")
        plt.xlabel("Epoch")
        plt.ylabel("CER (%)")
        plt.legend()
        plt.grid(True)
...
def run_experiment(exp_config: Dict[str, Any], 
    ...
    # NOTE: We use ignore_mismatched_sizes=True to allow replacing the old head
    # (which had Marathi vocab) with our NEW head (which has either Devanagari OR Latin vocab).
    model = Wav2Vec2ForCTC.from_pretrained(
        ...
        ignore_mismatched_sizes=True, # Critical for the swap
...
def main():
    print("=============================================")
    print("STARTING SAL MARATHI ORTHOGRAPHY EXPERIMENT")
    print("=============================================")
    ...
    # SAFETY CHECK: Ensure 'text' column exists, if 'sentence' exists, rename it.
    if "text" not in ds.column_names and "sentence" in ds.column_names:
        print("ℹ️  Renaming 'sentence' column to 'text' for consistency...")
